...
#            norm_fact = (abs(A)).sum()
        ...
        if(plot_type):
            if(plot_type == "rect"): # rectangular plot
                p1 = plt.subplot(111)
                p1.plot(u, F_plt, '-k',linewidth=1, label="$\mathrm{Bi-directional}$") # use "F.data" for unmasked F, if any exist                
                if(lattice): # highlighting visible-space and unit-lattice
                    plt.axvspan(-1, +1, facecolor='k', alpha=0.2)
                    lim = -np.pi / ((x[2] - x[1]) * k)                
                    plt.axvspan(-lim, +lim, facecolor='k', alpha=0.1)
...
a1 = 18e-3 # separation between elements along the x-axis in meters
...
u_scan = 1-(np.pi/(2*np.pi*a))
# Calling the 'pattern_u' function to evaluate and plot 2D AF/GF/NF
pattern_u2(array_ip, u_scan=u_scan, u_min=-1.2, u_max=1.2, u_num=1000, scale="dB",
      dB_limit= -40, factor="NF", plot_type="rect", lattice=True)

plt.annotate(r'$\mathrm{Grating}$  $\mathrm{Lobe}$', xy=(-0.5, -5),  xycoords='data',
